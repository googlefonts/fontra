...
export default class SelectionInfoPanel extends Panel {
  ...
  async update(senderInfo) {
    ...
    if (pointIndices.length >= 2) {
      // only allow path transformation if min two points are selected
      formContents.push(
        ...this._setupTransformationForPath(
          glyphController,
          pointIndices,
          componentIndices
        )
  ...
  async _scaleLayerGlyph(pointIndices, scaleFactorX, scaleFactorY) {
    ...
    formContents.push({
      ...
      fieldX: {
        ...
        setValue: (layerGlyph, layerGlyphController, fieldItem, value) => {
          ...
          //scaleOriginX = xMin + width / 2; // if scale from center
          //scaleOriginX = xMax; // if scale from right
          const pinPoint = { x: scaleOriginX, y: 0 };
          const scaleFactor = (100 + value) / 100;
          scaleFactorX = scaleFactor;

          for (const [i, index] of enumerate(range(0, layerGlyph.path.coordinates.length, 2))) {
            if (pointIndices.includes(i)) {
              let point = layerGlyph.path.getPoint(i);
              let pointScaled = scalePoint(pinPoint, point, scaleFactor);
              layerGlyph.path.coordinates[index] = pointScaled.x;
            }
          } */
        },
      ...
      fieldY: {
        ...
        setValue: (layerGlyph, layerGlyphController, fieldItem, value) => {
          ...
          scaleOriginY = yMin + height / 2; // if scale from center
          //scaleOriginY = yMax; // if scale from right
          const pinPoint = { x: 0, y: scaleOriginY };
          const scaleFactor = (100 + value) / 100;
          scaleFactorY = scaleFactor;

          for (const [i, index] of enumerate(range(1, layerGlyph.path.coordinates.length, 2))) {
            if (pointIndices.includes(i)) {
              let point = layerGlyph.path.getPoint(i);
              let pointScaled = scalePoint(pinPoint, point, scaleFactor);
              layerGlyph.path.coordinates[index] = pointScaled.y;
      ...
      auxiliaryElement: html.createDomElement("icon-button", {
        "style": `width: 1.3em;`,
        "src": "/tabler-icons/arrows-maximize.svg",
        "onclick": (event) =>
          this._scaleLayerGlyph(pointIndices, scaleFactorX, scaleFactorY),
        "data-tooltip": "Scale selected points",
        "data-tooltipposition": "left",
      }),
    });
